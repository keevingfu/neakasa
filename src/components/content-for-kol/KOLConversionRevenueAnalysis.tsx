...
import { TrendingUp, DollarSign, ShoppingCart, Target, Award, Users, PieChart, Clock, TrendingDown, Activity, Zap, Filter, Calendar, AlertTriangle, CheckCircle } from 'lucide-react';
...
const KOLConversionRevenueAnalysis: React.FC = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      console.log('KOLConversionRevenueAnalysis: Starting data load...');
      ...
      console.log('KOLConversionRevenueAnalysis: Data loaded successfully', { conversion, brands, roi });
      ...
      const transformedConversion = {
        ...
        revenueByCategory: Object.entries(conversion.revenue_by_category).map(([category, data]: [string, any]) => ({
          category,
          revenue: data.revenue,
          roi: data.roi
        })),
        monthlyRevenue: conversion.monthly_trend.map((item: any) => ({
      ...
      const transformedBrands = {
        topBrands: brands.map((brand: any) => ({
          brand: brand.brand,
      ...
      console.error('KOLConversionRevenueAnalysis: Error loading data:', error);
      ...
      series: [
        {
          ...
          data: conversionData.conversionFunnel.map((item: any, index: number) => ({
            value: item.value,
            name: item.stage,
            itemStyle: {
    ...
    return {
      ...
      tooltip: {
        ...
        formatter: (params: any) => {
          const data = params[0];
          return `${data.name}<br/>Revenue: $${(data.value / 1000).toFixed(0)}K<br/>ROI: ${data.data.roi}x`;
        }
      },
      grid: {
        left: '3%',
      ...
      yAxis: {
        type: 'category',
        data: conversionData.revenueByCategory.map((d: any) => d.category)
      },
      series: [
        {
          type: 'bar',
          data: conversionData.revenueByCategory.map((d: any) => ({
            value: d.revenue,
            roi: d.roi,
            itemStyle: {
              color: '#10b981'
          ...
          label: {
            show: true,
            position: 'right',
            formatter: (params: any) => `${params.data.roi}x ROI`
          }
        }
      ]
    };
  ...
  const getMonthlyRevenueTrendOption = () => {
    ...
    return {
      ...
      xAxis: {
        type: 'category',
        data: conversionData.monthlyRevenue.map((d: any) => d.month)
      },
      yAxis: [
        {
          type: 'value',
      ...
      series: [
        {
          name: 'Revenue',
          type: 'bar',
          data: conversionData.monthlyRevenue.map((d: any) => d.revenue),
        ...
        },
        {
          ...
          type: 'line',
          yAxisIndex: 1,
          data: conversionData.monthlyRevenue.map((d: any) => d.conversions),
          smooth: true,
  ...
  const getBrandMarketShareOption = () => {
    ...
    const data = Object.entries(brandAnalysis.competitorAnalysis).map(([brand, info]: [string, any]) => ({
      name: brand.charAt(0).toUpperCase() + brand.slice(1),
      value: info.marketShare,
      growth: info.growth
    }));
    ...
    return {
      ...
      tooltip: {
        trigger: 'item',
        formatter: (params: any) => {
          return `${params.name}<br/>Market Share: ${params.value}%<br/>Growth: ${params.data.growth}%`;
        }
      },
      series: [
        {
    ...
    <div className="space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl p-4 mb-6">
          <div className="flex items-start gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold text-gray-900 mb-1">Industry Benchmarks for Pet Products KOL Marketing</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
          ...
          <div className="bg-gradient-to-br from-pink-50 to-pink-100 p-4 rounded-xl">
            ...
            <div className="text-lg font-bold text-pink-900">
              {brandAnalysis?.topBrands?.[0]?.brand ? brandAnalysis.topBrands[0].brand.charAt(0).toUpperCase() + brandAnalysis.topBrands[0].brand.slice(1) : 'N/A'}
            </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm">
        <Tabs defaultValue="funnel" className="w-full">
          <TabsList className="grid w-full grid-cols-4 p-1 bg-gray-100 rounded-xl m-4">
            ...
            <TabsTrigger value="attribution" className="rounded-lg">Attribution Model</TabsTrigger>
            <TabsTrigger value="influencer" className="rounded-lg">Influencer Tiers</TabsTrigger>
            <TabsTrigger value="tracking" className="rounded-lg">Tracking Framework</TabsTrigger>
          </TabsList>
                ...
                <Card className="p-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex justify-between items-center py-2 border-b">
                      ...
                      <span className="text-sm font-medium">Views, Shares, Comments, Profile Visits</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b">
                      <span className="text-sm text-gray-600">Instagram</span>
                      <span className="text-sm font-medium">Reach, Saves, Story Views, Link Clicks</span>
                    </div>
                    <div className="flex justify-between items-center py-2 border-b">
                      <span className="text-sm text-gray-600">YouTube</span>
                      <span className="text-sm font-medium">Watch Time, Click-through Rate, End Screens</span>
                    </div>
                    <div className="flex justify-between items-center py-2">
                      <span className="text-sm text-gray-600">Xiaohongshu</span>
                      <span className="text-sm font-medium">Collections, Notes, Discovery Traffic</span>
                    </div>
                  </div>
                </Card>
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm">
        ...
        <div className="space-y-4">
          {[
            { phase: 'Phase 1: Foundation (Months 1-3)', tasks: ['Set up tracking infrastructure', 'Implement UTM parameters', 'Configure analytics platforms', 'Establish baseline metrics'], status: 'completed' },
            { phase: 'Phase 2: Optimization (Months 4-6)', tasks: ['A/B test content formats', 'Optimize influencer selection', 'Refine attribution models', 'Improve conversion funnels'], status: 'in-progress' },
            { phase: 'Phase 3: Scaling (Months 7-9)', tasks: ['Expand influencer network', 'Launch multi-platform campaigns', 'Implement automation tools', 'Scale successful strategies'], status: 'upcoming' },
            { phase: 'Phase 4: Advanced Analytics (Months 10-12)', tasks: ['Deploy predictive models', 'Implement real-time optimization', 'Create custom dashboards', 'Establish long-term partnerships'], status: 'upcoming' }
          ].map((phase, index) => (
                ...
                {phase.tasks.map((task, taskIndex) => (
                  <div key={taskIndex} className="flex items-start gap-2">
                    <CheckCircle className={`w-4 h-4 mt-0.5 ${
                      ...
                      phase.status === 'in-progress' && taskIndex < 2 ? 'text-blue-500' :
                      'text-gray-300'
                    }`} />
